...
export class FirstUserReactNativeSDK {
  ...
  init(config: FirstUserConfig) {
    this.config = {
      ...
      defaultClientPlatform: config.defaultClientPlatform ?? "react-native",
    };
  }

  startPresence(input: PresenceLoopOptions | ((status: PresenceStatus) => Promise<void>)) {
    if (!this.config) throw new Error("FirstUserReactNativeSDK not initialized");
    this.stopPresence();
    ...
    const statusProvider = input.statusProvider ?? (() => "live" as PresenceStatus);
    this.heartbeatTimer = setInterval(() => {
  ...
  async exchangeAccessCode(payload: AccessExchangePayload) {
    return this.postJson("/api/firstuser/access/exchange", {
      ...payload,
      clientPlatform: payload.clientPlatform ?? this.config?.defaultClientPlatform ?? "react-native",
    });
  }
